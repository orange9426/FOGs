...
class ReBeL(object):
    ...
    def learn(self):
        if len(self.replay_buffer) < self.batch_size or len(self.replay_buffer) < self.min_buffer_size:
            return None

        data = self.replay_buffer.sample(self.batch_size)
        pbses = torch.tensor([t[0] for t in data])
        values = torch.tensor([t[1] for t in data])
